...
for run in range(N_RUNS):
    ...
    c_ti  = m['car_targeting_index']
    u_ti  = m['unmod_targeting_index']
    c_arr  = m['car_mean_arrival']
    u_arr  = m['unmod_mean_arrival']

    car_targetings.append(c_ti)
    unmod_targetings.append(u_ti)
    car_arrivals.append(c_arr)
    unmod_arrivals.append(u_arr)
    car_wins.append(c_ti > u_ti)

    print(f"CAR ti={c_ti:.1f}%  Unmod ti={u_ti:.1f}%")

# ── Calculate statistics ──────────────────────────────────────────────────────
car_mean,   car_std   = np.mean(car_targetings),   np.std(car_targetings)
unmod_mean, unmod_std = np.mean(unmod_targetings), np.std(unmod_targetings)
...
targeting_in_range  = 40 <= car_mean <= 80

print("=" * 60)
print("TEST 07 — Metrics Validation (10 Runs)")
print("=" * 60)
print(f"CAR targeting mean ± std:   {car_mean:.1f} ± {car_std:.1f}%")
print(f"Unmod targeting mean ± std: {unmod_mean:.1f} ± {unmod_std:.1f}%")
...
print(f"Targeting in range [40,80]: {targeting_in_range}  (expect True)")
...
assert targeting_in_range, (
    f"FAIL: CAR mean targeting {car_mean:.1f}% outside [40, 80]"
)
...
axes[0].set_ylabel('Mean targeting index (%)', fontsize=11)
axes[0].set_title('Targeting Index\n(mean ± std, 10 runs)', fontsize=12)
...
axes[1].plot(x_vals, car_targetings,   'o-', color='#70a8e0',
             label='CAR-MuSC', linewidth=2, markersize=7)
axes[1].plot(x_vals, unmod_targetings, 's-', color='#e07070',
...
axes[1].set_ylabel('Targeting index (%)', fontsize=11)
axes[1].set_title('Targeting per run\n(dashed = mean)', fontsize=12)
